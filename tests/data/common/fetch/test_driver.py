...
def test_run_fetch_skips_a_file_already_on_hpc_when_transfer_mode_auto(tmp_path, monkeypatch):
    ...
    # downloaded (and then immediately pushed itself -- see
    # test_run_fetch_pushes_each_file_immediately_after_it_downloads).
    class _FakeClient:
        ...
        def rsync_transfer(self, source_path, target_path, source_is_local=True, options=None, show_progress=True):
            return True, "ok"
    ...
    # b.nc was genuinely outstanding, downloaded, then immediately pushed --
    # push_unit()'s default cleanup_local=True removes the local copy once
    # verified, so it's gone from local disk again by the time run_fetch()
    # returns.
    assert not os.path.exists(os.path.join(root, "b.nc"))


def test_run_fetch_pushes_each_file_immediately_after_it_downloads(tmp_path, monkeypatch):
    pushed = []
    ...
    class _FakeClient:
        ...
        def execute_command(self, command):
            return True, "", ""  # nothing on HPC yet -- both files are outstanding

        def ensure_directory(self, remote_path):
            return True

        def rsync_transfer(self, source_path, target_path, source_is_local=True, options=None, show_progress=True):
            pushed.append(target_path)
            return True, "ok"
    ...
    root = _raw_root(ctx, cfg)
    expected = {
        os.path.relpath(os.path.join(root, name), ctx.data_root).replace(os.sep, "/") for name in ("a.nc", "b.nc")
    }
    assert set(pushed) == expected  # both pushed during the run, not just at some later end-of-run step
    # push_unit()'s default cleanup_local=True removes the local copy once
    # pushed+verified -- confirms the push actually ran per file rather than
    # this test accidentally passing because nothing got pushed at all.
    assert not os.path.exists(os.path.join(root, "a.nc"))
    assert not os.path.exists(os.path.join(root, "b.nc"))
