...
# instead of falling back to discover()'s live crawl.
# ---------------------------------------------------------------------------
...
def test_discover_prepare_matches_old_plan_behavior(tmp_path):
    ...
    assert [t.key for t in targets] == ["2019", "2020", "2021"]
    assert targets[1].inputs == ("GL/Annual/V6GL02.04.CNNPM25.GL.202001-202012.nc4",)
    assert targets[1].meta["raw_relative_path"] == "GL/Annual/V6GL02.04.CNNPM25.GL.202001-202012.nc4"
...
def test_plan_prepare_reads_from_reconciled_ledger_not_disk(tmp_path):
    ...
    local_ledger_path = os.path.join(ctx.local_index_dir, "acag_pm25.duckdb")
