"""Regression test for the ease6933 grid-switch correctness fix.

Before this fix, AcagSource._execute_grid constructed SpatialProcessor with
no `target_geobox`, so it always reprojected onto the legacy EPSG:4326 grid
regardless of `ctx.grid_id` -- silently mislabeling output written under a
`stage_2_ease6933` path. This test is representative of every other
"Category A" source (eog, esacci, ntl_harm), which all follow the identical
SpatialProcessor(...) construction pattern in their own `_execute_grid`.

`get_target_geobox` itself is monkeypatched rather than exercised for real:
its own branching (legacy vs canonical) is covered by
tests/data/common/geobox/test_target_selection.py; the legacy branch needs a
real VIIRS download to build from, which this test has no reason to fake.
"""

import contextlib

import src.data.common.geobox as geobox_module
import src.data.common.raster.spatial as spatial_module
from src.data.pipeline.config import SourceConfig
from src.data.pipeline.context import PipelineContext
from src.data.sources.acag import AcagSource
from src.data.sources.steps import Completion, PipelineStep, StepTarget


class _FakeGeobox:
    def __init__(self, crs):
        self.crs = crs


class _SpySpatialProcessor:
    """Records the kwargs it was constructed with; never touches dask/xarray."""

    captured_kwargs = None

    def __init__(self, **kwargs):
        type(self).captured_kwargs = kwargs

    @contextlib.contextmanager
    def setup_dask_config(self):
        yield

    def process_spatial_standard(self, **kwargs):
        return True


def _make_source(tmp_path, grid_id):
    ctx = PipelineContext(
        data_root=str(tmp_path / "data_root"), local_index_dir=str(tmp_path / "index"), grid_id=grid_id
    )
    cfg = SourceConfig.from_dict("acag", {"data_path": "acag/pm25"})
    return AcagSource(ctx, cfg), ctx


def _run_execute_grid(tmp_path, grid_id, fake_geobox, monkeypatch):
    source, ctx = _make_source(tmp_path, grid_id)
    monkeypatch.setattr(spatial_module, "SpatialProcessor", _SpySpatialProcessor)
    monkeypatch.setattr(geobox_module, "get_target_geobox", lambda passed_ctx: fake_geobox)
    monkeypatch.setattr(type(source), "_dask_client", lambda self: contextlib.nullcontext("fake-client"))

    target = StepTarget(
        source_id=source.ID,
        step=PipelineStep.GRID,
        key="all",
        output_path=str(tmp_path / "out" / "acag_pm25_timeseries_reprojected.zarr"),
        inputs=("dummy_input.zarr",),
        completion=Completion.MARKER,
        meta={"years_available": [2019]},
    )
    result = source._execute_grid(target)
    assert result is True


def test_execute_grid_threads_legacy_geobox_through_to_spatial_processor(tmp_path, monkeypatch):
    fake_geobox = _FakeGeobox(crs="EPSG:4326")
    _run_execute_grid(tmp_path, "legacy_4326", fake_geobox, monkeypatch)
    assert _SpySpatialProcessor.captured_kwargs["target_geobox"] is fake_geobox


def test_execute_grid_threads_canonical_ease_geobox_through_to_spatial_processor(tmp_path, monkeypatch):
    fake_geobox = _FakeGeobox(crs="EPSG:6933")
    _run_execute_grid(tmp_path, "ease6933", fake_geobox, monkeypatch)
    assert _SpySpatialProcessor.captured_kwargs["target_geobox"] is fake_geobox
