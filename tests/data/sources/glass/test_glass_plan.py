...
def _make_source(tmp_path, source_id, **extra_raw):
    ctx = PipelineContext(
...
def test_resolve_source_file_path_matches_fetch_output_root(tmp_path):
    ...
    assert source._resolve_source_file_path("foo/bar.hdf") == os.path.join(
        ctx.data_root, "raw", "glass/LST/MODIS/Daily/1KM/", "foo/bar.hdf"
    )
...
def test_output_path_uses_family(tmp_path):
    modis_source, modis_ctx = _make_source(tmp_path, "glass_modis")
    assert modis_source._grid_output_path() == os.path.join(modis_ctx.data_root, "grid", "legacy_4326", "glass_modis_lst.zarr")

    avhrr_source, avhrr_ctx = _make_source(tmp_path, "glass_avhrr")
