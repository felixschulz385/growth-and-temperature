...
def _make_source(tmp_path, layout_name="legacy", **extra_raw):
    ctx = PipelineContext(
        data_root=str(tmp_path / "data_root"), local_index_dir=str(tmp_path / "index"), layout=layout_name
    )
...
def test_resolve_source_file_path_matches_fetch_output_root(tmp_path):
    ...
    legacy_source, legacy_ctx = _make_source(tmp_path, layout_name="legacy")
    assert legacy_source._resolve_source_file_path("foo/bar.hdf") == os.path.join(
        legacy_ctx.data_root, "glass/LST/AVHRR/0.05D/", "raw", "foo/bar.hdf"
    )

    v2_source, v2_ctx = _make_source(tmp_path, layout_name="v2")
    assert v2_source._resolve_source_file_path("foo/bar.hdf") == os.path.join(
        v2_ctx.data_root, "raw", "glass/LST/AVHRR/0.05D/", "foo/bar.hdf"
    )
