#!/bin/bash
#SBATCH --job-name=assemble_update
#SBATCH --output=./log/assemble/update/%x-%j.out
#SBATCH --error=./log/assemble/update/%x-%j.err
#SBATCH --partition=scicore
#SBATCH --time=1-00:00:00
#SBATCH --qos=1day
#SBATCH --cpus-per-task=4
#SBATCH --mem=128G

mkdir -p "./log/assemble/update"

# Accept assemble config name as first argument, default to "modis"
SOURCE=${1:-'5km'}

# Accept data source to update as second argument, default to "modis"
DATASOURCE=${2:-'modis'}

# Activate conda environment
eval "$(/scicore/home/meiera/schulz0022/miniforge-pypy3/bin/conda shell.bash hook)"
conda activate src

# Change to project directory
cd /scicore/home/meiera/schulz0022/projects/growth-and-temperature

# Set DATA_NOBACKUP environment variable
export DATA_NOBACKUP="/scicore/home/meiera/schulz0022/projects/growth-and-temperature/data_nobackup"

# Calculate memory limit (leave some buffer for system - 60% of allocated)
MEMORY_LIMIT_GB=$(echo "scale=0; $SLURM_MEM_PER_NODE * 0.6 / 1024" | bc)

# Run the assembly using the unified interface
python run.py assemble update \
    --datasource $DATASOURCE \
    --config orchestration/configs/data.yaml \
    --source $SOURCE \
    --dask-threads $SLURM_CPUS_PER_TASK \
    --dask-memory-limit "${MEMORY_LIMIT_GB}GiB" \
    --temp-dir "${DATA_NOBACKUP}/scratch/dask" \
    --dashboard-port 8787 \
    --debug
