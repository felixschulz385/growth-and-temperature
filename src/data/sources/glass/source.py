    ...
    def _plan_grid(self, selection: TargetSelection) -> List[StepTarget]:
        ...
        if self.data_source_kind == "MODIS":
            return [
                StepTarget(
                    ...
                    key="all_cells",
                    output_path=os.path.join(self.output_root(PipelineStep.GRID), "modis_timeseries_reprojected.zarr"),
        ...
        return [
            StepTarget(
                ...
                step=PipelineStep.GRID,
                key="global",
                output_path=os.path.join(self.output_root(PipelineStep.GRID), "avhrr_timeseries_reprojected.zarr"),
                inputs=tuple(f["zarr_path"] for f in annual_files),
                completion=Completion.MARKER,
