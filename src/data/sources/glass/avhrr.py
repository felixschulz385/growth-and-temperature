...
class GlassAvhrrSource(DataSource):
    ...
    def output_root(self, step: PipelineStep, *, namespace: str | None = None) -> str:
        ...
        `agg=CRS_AGG` for PREPARE specifically: under `layout="v2"`,
        `layout.output_root()` now requires an explicit `agg` for PREPARE
        (`src/data/sources/layout.py`'s CRS_AGG/ADM_AGG/MISC_AGG physical-
        layout split, added independently of this rebuild). GLASS-AVHRR's
        PREPARE output (`_annual_zarr_path()`'s per-year daily->annual
        stats zarr) is a pixel-grid raster store -- just not yet reprojected
        onto the canonical grid, which is what the final `_grid_output_path()`
        (routed through GRID, not PREPARE, so it never needs `agg`) does --
        so CRS_AGG is the locally-consistent bucket, unambiguous here since
        AVHRR has no non-pixel-grid PREPARE output that would need ADM_AGG/
        MISC_AGG instead."""
