...
class NTLHarmPreprocessor(AbstractPreprocessor):
    """
    HPC-mode preprocessor for harmonized nighttime lights (NTL) data.
    
    This class handles the preprocessing of harmonized DMSP-VIIRS nighttime lights data
    from the Figshare dataset. The preprocessing involves:
    1. Processing raw data files into annual composites
    2. Saving the results as zarr files for efficient access
    
    The preprocessing is organized in three stages:
    - Stage 1: Create annual zarr files from raw data
    - Stage 2: Reproject the data to a unified grid for analysis
    - Stage 3: Convert to tabular format for analysis
    
    Requires HPC mode configuration with parquet index.
    """
    
    # Variable name for the harmonized data
    VARIABLE_NAME = "ntl_harm"
    
    def __init__(self, **kwargs):
        """
        Initialize the NTL Harm preprocessor in HPC mode.
        
        Args:
            **kwargs: Configuration parameters including:
                stage (str): Processing stage ("annual", "spatial", "tabular")
                year (int, optional): Specific year to process
                year_range (list, optional): [start_year, end_year] range to process
                
                # Data source parameters
                base_url (str): Base URL for the Figshare API
        ...
        if not self.temp_dir:
            self.temp_dir = tempfile.mkdtemp(prefix=f"ntl_harm_processor_")
        else:
            os.makedirs(self.temp_dir, exist_ok=True)
            
        logger.info(f"Initialized NTLHarmPreprocessor")
    ...
    def _generate_annual_targets(self, files: List[str], year_range: Tuple[int, int] = None) -> List[Dict]:
        ...
        for year, year_files in files_by_year.items():
            # For harmonized NTL data, we expect one file per year
    ...
    def _extract_year_from_path(self, file_path: str) -> Optional[int]:
        ...
        for pattern in year_patterns:
            ...
            if match:
                ...
                # Validate reasonable year range for harmonized NTL data (1992-2018)
                if 1992 <= year <= 2030:
                    return year
    ...
    def _select_best_file_for_year(self, year_files: List[str]) -> str:
        """Select the best file for a given year (prefer .tif over compressed formats)."""
        if len(year_files) == 1:
            return year_files[0]
        
        # Preference order: .tif > .zip > .tar.gz > .gz
        preference_order = ['.tif', '.zip', '.tar.gz', '.gz']
        
        for ext in preference_order:
            for file_path in year_files:
                if file_path.lower().endswith(ext):
                    return file_path
        
        # If no preferred extension found, return the first file
        return year_files[0]
    ...
    def _generate_spatial_targets(self, files: List[str], year_range: Tuple[int, int] = None) -> List[Dict]:
        ...
        target = {
            'data_type': 'ntl_harm_spatial',
            'stage': 'spatial',
            'source_files': [f['zarr_path'] for f in annual_files],
            'output_path': f"{self.get_hpc_output_path('spatial')}/ntl_harm_timeseries_reprojected.zarr",
    ...
    def _process_annual_target(self, target: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.exception(f"Error in NTL Harm annual processing: {e}")
            return False
    ...
    def _process_data_files(self, file_path: str, year: int) -> xr.DataArray:
        """Process NTL Harm data files into xarray format."""
        if not file_path:
            return None
        ...
        try:
            ...
            if file_path.endswith(".gz"):
                ...
                # Find the extracted file (assume single file in zip)
                extracted_files = [f for f in os.listdir(extract_dir) if f.endswith('.tif')]
                if extracted_files:
                    local_file = os.path.join(extract_dir, extracted_files[0])
                    uncompressed_file_to_delete = extract_dir  # Will remove whole directory
                else:
            ...
            logger.info(f"Opening NTL Harm file with Dask chunks: {local_file}")
            ...
            logger.info(f"Successfully loaded NTL Harm data with shape {ds.shape} and chunks {ds.chunks}")
            return ds
    ...
    def _create_annual_zarr_hpc(self, data_array: xr.DataArray, output_path: str) -> bool:
        ...
        try:
            ...
            dataset = data_array.to_dataset(name=self.VARIABLE_NAME)
            ...
            logger.info(f"Writing NTL Harm dataset to zarr with Dask: {output_path}")
            dataset.to_zarr(
                output_path, 
                mode="w", 
                encoding=encoding
            ...
            # Now it's safe to clean up the uncompressed file/directory
        ...
        try:
            with self._initialize_dask_client() as client:
                ...
                with spatial_processor.setup_dask_config():
                    ...
                    # Define NTL Harm-specific functions
                    def extract_year_from_ntl_harm_path(file_path: str) -> Optional[int]:
                        """Extract year from NTL Harm zarr file path."""
                        try:
                            return int(os.path.splitext(os.path.basename(file_path))[0])
                        except ValueError:
                            return None
                    
                    def preprocess_ntl_harm_dataset(ds: xr.Dataset) -> xr.Dataset:
                        """Preprocess NTL Harm dataset for reprojection."""
                        # Set CRS if needed (harmonized data should be in WGS84)
                        if ds.rio.crs is None:
                            ds = ds.rio.write_crs(4326)
                        return ds
                    
                    def get_ntl_harm_variables_and_attrs(file_path: str) -> Tuple[List[str], Dict]:
                        """Get variables and attributes from NTL Harm sample file."""
                    ...
                    if success:
                        logger.info("NTL Harm spatial stage processing completed successfully")
                    
                    return success
                        
        except Exception as e:
            logger.exception(f"Error in NTL Harm spatial processing: {e}")
            return False
    ...
    @classmethod
    def from_config(cls, config: Dict[str, Any]) -> 'NTLHarmPreprocessor':
